...
if(isset($_POST['auto']))
{
	if (!empty($_POST['login']) && isset($_POST['pass'])) {
		if (@mysql_connect(DBHOST, $_POST['login'], $_POST['pass'])){
			$my_logpwd = base64_encode("SKD101:{$_POST['login']}:{$_POST['pass']}");
			mysql_close();
		}
		else{
		...
		$user = explode(":", base64_decode($my_logpwd));
		...
		if(isset($_POST['fileleave'])) $fileLeaveCount = $_POST['fileleave'];
	}
	else{
		$error = "login not set";
	}
...
class dumper {
	...
	function backup() {
		...
		echo "--==".$fileLeaveCount."==--";
		if(isset($fileLeaveCount)) delOldFile(PATH, $fileLeaveCount);
	}
	...
	function restore(){
		...
		while(($str = $this->fn_read_str($fp)) !== false){
			...
			if ($sql) {
				...
				if ($execute) {
					$q++;
					mysql_query($sql) or trigger_error ("Неправильный запрос.<BR>" . mysql_error().":".$sql, E_USER_ERROR);
...
function delOldFile($path, $filecount)
{
	$num = 0;
	$fileslist = array();
	...
	if ($handle = opendir($path)){
		while (false !== ($file = readdir($handle))){ 
			if ($file != "." && $file != ".." && preg_match('/^(.+)\.gz$/', $file)){ 
				$fileslist[$file] = filectime($path.$file);	
			} 
		}
		closedir($handle);
		
		array_multisort ($fileslist, SORT_DESC);
